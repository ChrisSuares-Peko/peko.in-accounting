...
const pekoCreditsSchema = Yup.object().shape({
    ...
    vendorDiscount: Yup.number()
        .min(0, 'Vendor discount cannot be negative')
        .when('discount', (discount: any, schema: any) =>
            discount
                ? schema.max(Number(discount), 'Vendor discount must be less than or equal to discount')
                : schema
        )
        .notRequired()
        .nullable(),
