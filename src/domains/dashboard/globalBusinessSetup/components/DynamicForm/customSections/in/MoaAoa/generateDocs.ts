...
import { buildTemplateData, resolveMoaInputs } from './resolve';
...
/**
 * Build the MOA/AOA PDFs from the final submitted values and write them onto the
 * section's `generated_*_file` fields. Runs at submit time (not tied to the
 * component being mounted), so the documents reflect the latest data regardless
 * of which page the user last viewed. Mutates `fields` in place.
 */
export async function generateMoaAoaDocs(
    ...
    const { templateData, companyName } = buildTemplateData(values, config, countries);
