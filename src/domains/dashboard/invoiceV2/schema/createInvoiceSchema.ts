...
export const createInvoiceSchema = Yup.object({
    ...
    invoice: Yup.object({
        ...
        dateOfSupply: Yup.string()
            .optional()
            .nullable()
            .test('date-of-supply-format', 'Date of supply must be in YYYY-MM-DD format', v => {
                if (!v) return true;
    ...
    items: Yup.array()
        .of(
            Yup.object({
                name: withLetterRequired(
                    withSpaceValidation(
                        Yup.string().required('Please enter an item title'),
                ...
                hsn: Yup.string().test(
                    'hsn-validation',
                    'HSN code must be 2, 4, 6, or 8 numeric digits',
                    v => {
                        if (!v) return true;
