...
const NavigationCards = ({ icon, title, link, isActive, reference, onClick, iconWidth = 40 }: CardProps) => {
    ...
    const desktopCard = (
        ...
        >
            <Flex className="h-24 w-24 bg-bgIconCard rounded-3xl" align="center" justify="center">
                <Image width={iconWidth} preview={false} src={icon} />
            </Flex>
            <Typography.Text
    ...
    const mobileCard = (
        ...
        >
            <Flex className="min-w-[4rem] h-14 bg-bgIconCard rounded-xl justify-center">
                <Image width={iconWidth / 2} preview={false} src={icon} className="mt-5" />
            </Flex>
            <Typography.Text
                {...(!isActive && { disabled: true })}
