...
describe('UserModal', () => {
    ...
    // Role is an authorization input — 'Admin' grants corporate-wide card authority — so it must be chosen
    // from the two allowed values, never typed. A free-text box here would let a job title become a grant.
    it('offers Role as a fixed choice, not a free-text field', async () => {
        renderComponent();

        await waitFor(() => {
            expect(screen.getByText('Role')).toBeInTheDocument();
        });
        expect(screen.queryByPlaceholderText('Enter employee role')).toBeNull();
    ...
    it('should render service selection checkboxes when isInitialSubmit is false', async () => {
        ...
        expect(screen.getByText('Enable All Services')).toBeInTheDocument();
    ...
    it('should handle form submission correctly', async () => {
        ...
        // Role needs no interaction: it defaults to Employee, the least-privileged value.
        ...
        fireEvent.change(screen.getByPlaceholderText('Confirm your email ID'), {
            target: { value: 'john@example.com' },
        });
